...
function load_agilent(
    ...
    if aggregate_all === false && sample === nothing
        throw(ArgumentError("Either `sample` needs to be specified or aggregate_all needs to be true"))
    end
    data = DataFrame()
    ...
    for file in files
        ...
        analysis_time = chop(
            ...
            tail = length(df[3, 1]) - findnext(" u", df[3, 1], findfirst(":", df[3, 1])[1])[1] + 1,
        )
        ...
        df.ratio_mdn_norm = df.ratio ./ median(df[stable_time .< df.time .< signal_end, :ratio])
