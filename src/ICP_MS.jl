...
-  `sample` only needs to be a unique portion of the sample name. E.g. NIST610 will just load `NIST610*.csv` files, whereas NIST will load any file where the name contains `NIST*.csv`).
- `cps_columnX` should be specified as `element` then `isotopic mass` (e.g., `Rb85`). It matches using regular expression and normalises the header names. If duplicate isotope values are present (due to mass shift checks, e.g. K39 -> 39, K39 -> 58) specify the desired mass by replacing ` -> ` with `_` (e.g. `K39 -> 39` to `K39_39`).

# Keywords
- `firstrow::Int`: An `Int` representing the first data row. Default value is `5`. This can be used to remove the rows prior to `stabletime` if set to an appropriate value (e.g, ~110 for a 30 second gas blank).
- `stabletime::Real`: The time (in seconds) where signal counts are stabilised. Default value is `32` (i.e. 32 seconds). This parameter is used to filter the rows when calculating the median counts per second ratio.
- `signalend::Real`: The time (in seconds) where the data should be truncated. Default value is `Inf` (will use entire signal beyond `stabletime`). It is recommended to set this to a value appropriate to your data (i.e. total signal time minus ~5 [e.g. 65]) to avoid some artefacts  that may occur near the end of an ablation.
- `trim::Bool`: Whether to trim the data to only contain values between `firstrow` and `signalend`. Default value is `false`. Setting to `true` it will delete all data from the first record past `signalend` to the last record.
